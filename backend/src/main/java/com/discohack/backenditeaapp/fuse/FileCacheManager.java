...
/**
 * FileCacheManager — дисковый кэш содержимого файлов из облака.
 *
 * Как работает:
 * 1. При первом чтении файла через FUSE скачивается весь файл и сохраняется в
 *    ~/.cache/discohack/files/{accountId}/{sha256-пути}
 * 2. При повторном чтении (любого куска) — данные берутся с диска, сеть не нужна.
 * 3. При изменении или удалении файла — кэш инвалидируется.
 * 4. Когда суммарный размер кэша превышает лимит из настроек — вытесняются
 *    наименее недавно использованные файлы (LRU).
 *
 * Структура на диске:
 *   ~/.cache/discohack/files/
 *     {accountId}/
 *       {sha256-от-пути}.bin   ← содержимое файла
 */
@Slf4j
@Component
